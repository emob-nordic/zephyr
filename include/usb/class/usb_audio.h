 ...
 */
enum usb_audio_feature_unit_control_selectors {
...
struct usb_audio_feature_unit_evt {
	enum usb_audio_direction dir;
	enum usb_audio_feature_unit_control_selectors cs;
	u8_t channel;
	void *val;
};
 ...
 *
 * @param evt Event information to be parsed by app.
 */
typedef void (*usb_audio_feature_updated_cb_t)(struct device *dev,
				const struct usb_audio_feature_unit_evt evt);
